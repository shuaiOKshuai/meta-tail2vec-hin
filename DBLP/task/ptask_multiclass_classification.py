...
dataset = 'dblp'
train_dir = './data/' + dataset + '/dblp_han.txt'
test_dir_1 = './data/' + dataset + '/dblp_han.txt'
test_dir_3 = './data/' + dataset + '/dblp_han.txt'
test_dir_5 = './data/' + dataset + '/dblp_han.txt'


def read_node_class():
    nodes_list = list()
    node_class_dict = dict()
    node_class_num_dict = dict()
    ...
    with open('./data/' + dataset + '/node_type.txt', "r") as f:
        lines = f.readlines()
        for line in lines:
            temp = list(line.strip('\n').split(' '))
            nodes_list.append(temp[0])
    with open('./data/' + dataset + '/gt_p.txt', "r") as f:
        lines = f.readlines()
        for line in lines:
            temp = list(line.strip('\n').split(' '))
            # nodes_list.append(temp[0])
            node_class_dict[temp[0]] = temp[1]
    test_nodes_list_new = list()
    for n in test_nodes_list:
        if n in node_class_dict:
            test_nodes_list_new.append(n)
    for n in test_nodes_list_new:
        if node_class_dict[n] not in node_class_num_dict:
            node_class_num_dict[node_class_dict[n]] = 0
        node_class_num_dict[node_class_dict[n]] += 1
    max_label_num = -1
    max_index = '-1'
    for c in node_class_num_dict:
        if node_class_num_dict[c] > max_label_num:
            max_index = c
            max_label_num = node_class_num_dict[c]
    mi_f1_labels = list()
    for c in node_class_num_dict:
        if c != max_index:
            mi_f1_labels.append(int(c))
    nodes4train_list = list()
    for n in nodes_list:
        if n not in test_nodes_list and n in node_class_dict:
            nodes4train_list.append(n)
    print(len(nodes4train_list))
    print(len(test_nodes_list_new))
    return nodes4train_list, test_nodes_list_new, node_class_dict, mi_f1_labels
...
if __name__ == '__main__':
    nodes4train, test_nodes, node_class, mi_f1_labels = read_node_class()
    ...
    for s in range(num_splits):
        ...
        for average in averages:
            if average == "micro":
                results[average] = f1_score(y_test, preds, average=average, labels=mi_f1_labels)
            else:
        ...
        for average in averages:
            if average == "micro":
                results1[average] = f1_score(y_test, preds1, average=average, labels=mi_f1_labels)
            else:
        ...
        for average in averages:
            if average == "micro":
                results3[average] = f1_score(y_test, preds3, average=average, labels=mi_f1_labels)
            else:
        ...
        for average in averages:
            if average == "micro":
                results5[average] = f1_score(y_test, preds5, average=average, labels=mi_f1_labels)
            else:
