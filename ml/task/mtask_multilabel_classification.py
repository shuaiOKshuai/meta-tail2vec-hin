...
dataset = 'ml'
train_dir = './data/' + dataset + '/ml_han.txt'
test_dir_1 = './data/' + dataset + '/additive-1-han.txt'
test_dir_3 = './data/' + dataset + '/additive-2-han.txt'
test_dir_5 = './data/' + dataset + '/ml_han.txt'
feature_length = 128
label_length = 0
...
def read_node_class():
    ...
    with open('./data/' + dataset + '/node_type.txt', "r") as f:
        ...
        for line in lines:
            ...
            nodes_list.append(temp[0])
    label_max = 0
    with open('./data/' + dataset + '/gt_m.txt', "r") as f:
        lines = f.readlines()
        for line in lines:
            temp = list(line.strip('\n').split(' '))
            # nodes_list.append(temp[0])
            if temp[0] not in node_class_dict:
                node_class_dict[temp[0]] = list()
            for g in temp[1:]:
                if int(g) > label_max:
                    label_max = int(g)
                node_class_dict[temp[0]].append(int(g))
    label_length = label_max + 1
    for n in node_class_dict.keys():
        label = [0.] * label_length
        for col in node_class_dict[n]:
            label[int(col)] = 1.0
    ...
    return nodes4train_list, test_nodes_list_new, node2label_dict, node_class_dict, label_length
...
if __name__ == '__main__':
    nodes4train, test_nodes, node2label, node_class, ll = read_node_class()
    label_length = ll
