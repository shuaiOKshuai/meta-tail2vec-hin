...
from data_generator_f import DataGenerator
from maml_8 import MAML
...
def train(model, sess, writer):
    ...
    for iteration in range(10000):
        ...
        result = sess.run(ops)
        if iteration % 20 == 0:
            writer.add_summary(result[1], iteration)
...
def test1(model, sess, dataset_name, iters):
    test_preds = []
    fw_5 = open('./data/' + dataset_name + '/result_5_' + str(iters) + '.csv', 'w')
    fw_3 = open('./data/' + dataset_name + '/result_3_' + str(iters) + '.csv', 'w')
    fw_1 = open('./data/' + dataset_name + '/result_1_' + str(iters) + '.csv', 'w')
    for i in range(3500):
        if i % 100 == 1:
            print(i)
        ops = [model.test_query_preds, model.query_nodes]
...
def main():
    ...
    db = DataGenerator(main_dir, dataset_name, kshot, meta_batchsz, class_num, 25000)
    if training:
